...
@section('contenido')
    <div class="flex-none w-full px-3">
        ...
        <div class="flex justify-end mb-4">
          <a class="px-4 py-2 text-sm font-medium text-white bg-blue-500 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600" href="{{ route('formCategorias') }}">
                  ...
                  <table id="myTable" class="items-center table-auto mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    ...
                    @foreach ($categorias as $categorias)
                      <tr>
                        ...
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <a href="javascript:;" class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{{ $categorias->descripcion }}</a>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <a href="javascript:;" class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{{ $categorias->creador->name }}</a>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent" style="margin-right: 8px; margin-left:8px;">
                            <a href="{{ route('categorias.editarCategoria', $categorias->id) }}" class="text-blue-500 hover:text-blue-700 rounded-full bg-blue-500 text-white p-2">
                                <i class="fas fa-pencil-alt"></i>
                            </a>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <form action="{{ route('categorias.delete', $categorias->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
